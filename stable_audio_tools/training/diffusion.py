...
class DiffusionUncondDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            log_dict[f"demo"] = wandb.Audio(filename, sample_rate=self.sample_rate, caption=f"Reconstructed")
            ...
            if not self.pre_encoded:
                with torch.cuda.amp.autocast() and torch.set_grad_enabled(self.diffusion.pretransform.enable_grad):
                    ...
                    if use_padding_mask:
                        padding_masks = (
                            F.interpolate(
                                padding_masks.unsqueeze(1).float(), size=diffusion_input.shape[2], mode="nearest"
                            )
                            .squeeze(1)
                            .bool()
                        )
    ...
    def on_train_batch_end(self, trainer, module: DiffusionCondTrainingWrapper, outputs, batch, batch_idx):
        ...
        try:
            ...
            if self.display_audio_cond:
                ...
                log_dict[f"demo_audio_cond"] = wandb.Audio(
                    filename, sample_rate=self.sample_rate, caption="Audio conditioning"
                )
            ...
            for cfg_scale in self.demo_cfg_scales:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if module.diffusion_objective == "v":
                        fakes = sample(
                            model, noise, self.demo_steps, 0, **cond_inputs, cfg_scale=cfg_scale, batch_cfg=True
                        )
                    elif module.diffusion_objective == "rectified_flow":
                        fakes = sample_discrete_euler(
                            model, noise, self.demo_steps, **cond_inputs, cfg_scale=cfg_scale, batch_cfg=True
                        )
    ...
    def on_train_batch_end(self, trainer, module: DiffusionCondTrainingWrapper, outputs, batch, batch_idx):
        ...
        try:
            ...
            if not module.pre_encoded:
                # Log the real audio
                log_dict[f"demo_reals_melspec_left"] = wandb.Image(
            ...
            if module.diffusion.pretransform is not None:
                ...
                log_dict[f"demo_masked_input"] = wandb.Image(
        ...
        if module.diffae_ema.ema_model.pretransform is not None:
            with torch.no_grad() and torch.cuda.amp.autocast():
                ...
                log_dict[f"first_stage"] = wandb.Audio(
                    first_stage_filename, sample_rate=self.sample_rate, caption=f"First Stage Reconstructed"
                )
...
class DiffusionPriorTrainingWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        log_dict = {"train/loss": loss.detach(), "train/std_data": reals.std()}
...
class DiffusionPriorDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module: DiffusionAutoencoderTrainingWrapper, outputs, batch, batch_idx):
        ...
        log_dict[f"source"] = wandb.Audio(filename, sample_rate=self.sample_rate, caption=f"Source")
