...
import torchaudio
import wandb
from aeiou.viz import audio_spectrogram_image, pca_point_cloud, tokens_spectrogram_image
...
from .losses import AuralossLoss, L1Loss, MultiLoss, ValueLoss
...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
        ...
        if optimizer_configs is None:
            optimizer_configs = {
                "autoencoder": {"optimizer": {"type": "AdamW", "config": {"lr": lr, "betas": (0.8, 0.99)}}},
                "discriminator": {"optimizer": {"type": "AdamW", "config": {"lr": lr, "betas": (0.8, 0.99)}}},
            }
        ...
        if loss_config is None:
            ...
            loss_config = {
                "discriminator": {
                    "type": "encodec",
                    "config": {"n_ffts": scales, "hop_lengths": hop_sizes, "win_lengths": win_lengths, "filters": 32},
                    "weights": {
                        "adversarial": 0.1,
        ...
        if loss_config["discriminator"]["type"] == "oobleck":
            ...
            self.discriminator = EncodecDiscriminator(
                in_channels=self.autoencoder.out_channels, **loss_config["discriminator"]["config"]
            )
        ...
        self.gen_loss_modules += [
            ValueLoss(
                key="loss_adv", weight=self.loss_config["discriminator"]["weights"]["adversarial"], name="loss_adv"
            ),
            ValueLoss(
        ...
        if self.use_ema:
            self.autoencoder_ema = EMA(
                self.autoencoder, ema_model=ema_copy, beta=0.9999, power=3 / 4, update_every=1, update_after_step=1
            )
    ...
    def configure_optimizers(self):
        ...
        opt_gen = create_optimizer_from_config(
            self.optimizer_configs["autoencoder"]["optimizer"], self.autoencoder.parameters()
        )
        opt_disc = create_optimizer_from_config(
            self.optimizer_configs["discriminator"]["optimizer"], self.discriminator.parameters()
        )
...
class AutoencoderDemoCallback(pl.Callback):
    def __init__(self, demo_dl, demo_every=2000, sample_size=65536, sample_rate=48000):
        super().__init__()
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            log_dict[f"recon"] = wandb.Audio(filename, sample_rate=self.sample_rate, caption=f"Reconstructed")
