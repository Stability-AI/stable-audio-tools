...
from .transformer import Attention, FeedForward, LayerNorm, RotaryEmbedding
            ...
            self.layers.append(
                nn.ModuleList(
                    [
                        ...
                        (
                            Attention(
                                dim=dim, dim_heads=dim_head, dim_context=cross_attn_cond_dim, zero_init_output=True
                            )
                            if self.cross_attn_cond_dim > 0
                            else nn.Identity()
                        ),
                        AdaRMSNorm(dim, cond_dim, eps=1e-8) if cond_dim > 0 else LayerNorm(dim),
...
class TransformerDownsampleBlock1D(nn.Module):
    def __init__(
        self, in_channels, embed_dim=768, depth=3, heads=12, downsample_ratio=2, local_attn_window_size=64, **kwargs
    ):
        super().__init__()
        ...
        self.transformer = ContinuousLocalTransformer(
            dim=embed_dim, depth=depth, heads=heads, local_attn_window_size=local_attn_window_size, **kwargs
        )
...
class TransformerUpsampleBlock1D(nn.Module):
    def __init__(
        self, in_channels, embed_dim, depth=3, heads=12, upsample_ratio=2, local_attn_window_size=64, **kwargs
    ):
        super().__init__()
