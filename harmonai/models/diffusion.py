...
def create_diffusion_uncond_from_config(model_config: Dict[str, Any]):
    ...
    if model_type == 'DAU1d':
        return DiffusionAttnUnet1D(
            ...
            # depth = model_config.get('depth', 14),
            # n_attn_layers = model_config.get('n_attn_layers', 6),
            # channels = model_config.get('channels', [128, 128, 256, 256] + [512] * 10),
            # cond_dim = model_config.get('cond_dim', 0),
            # cond_noise_aug = model_config.get('cond_noise_aug', False),
            # kernel_size = model_config.get('kernel_size', 5),
            # learned_resample = model_config.get('learned_resample', False),
            # strides = model_config.get('strides', [2] * 14),
